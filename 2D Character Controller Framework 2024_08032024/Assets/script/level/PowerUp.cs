...
public class PowerUp : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (col.gameObject == _player)
        {
            ...
            if (powerUpType == PowerUpType.DoubleJump)
            {
                ...
                StartCoroutine(DeactivatePowerUp(_playerMovement, 5f));
            }
    ...
    IEnumerator DeactivatePowerUp(PlayerMovement playerMovement, float duration)
    {
        ...
        // Deactivate the corresponding power after the specified duration.
        if (powerUpType == PowerUpType.DoubleJump)
        {
            playerMovement.doubleJumpOn = false;
        }
        else if (powerUpType == PowerUpType.WallJump)
        {
            playerMovement.wallJumpOn = false;
        }
        else if (powerUpType == PowerUpType.Glide)
        {
            playerMovement.glideOn = false;
        }
        else if (powerUpType == PowerUpType.Shoot)
        {
            playerMovement.shootOn = false;
        }
    }
}
